...
type Layer private (edges: Edge list, lines: LineSegment list, points: Point list, layerType: LayerType) =
    ...
    /// 指定した要素をもつレイヤーを生成します。
    static member Create(edges: seq<Edge>, lines: seq<LineSegment>, points: seq<Point>, layerType) =
    ...
    static member FromPolygon(vertexes: seq<Point>, layerType) =
        ...
        let rec createEdges (head: Point) (points: Point list) (acm: Edge list) =
            ...
            | [] -> acm
        Layer(createEdges vertexes.Head vertexes [], [], vertexes, layerType)

    static member AsLayer(source: ILayer) =
        match source with
        | :? Layer as ly -> ly
        | _ -> Layer(asList source.Edges, asList source.Lines, asList source.Points, source.LayerType)
