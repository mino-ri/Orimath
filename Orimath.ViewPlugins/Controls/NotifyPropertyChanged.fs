...
type NotifyPropertyChanged() =
    ...
    member this.OnPropertyChanged([<Optional; DefaultParameterValue(""); CallerMemberName>] propertyName) =
        ev.Trigger(this, PropertyChangedEventArgs(propertyName))

    member this.SetValue<'T>(storage: byref<'T>, value, [<Optional; DefaultParameterValue(""); CallerMemberName>] propertyName) =
